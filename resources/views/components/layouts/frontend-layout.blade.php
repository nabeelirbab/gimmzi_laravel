...
@if (Auth::check())
    @if (Auth::user()->hasRole('MERCHANT'))
        <x-frontend.merchant.header />
    @endif
    @if (Auth::user()->hasRole('PROVIDER'))
        <x-frontend.provider.header />
    @endif
    @if (Auth::user()->hasRole('CONSUMER'))
        <x-frontend.consumer.header />
    @endif
    @if (Auth::user()->hasRole('SHORT TERM RENTAL PROVIDER'))
        <x-frontend.travel_tourism.header />
    @endif
    @if (Auth::user()->hasRole('HOTEL RESORT PROVIDER'))
        <x-frontend.travel_tourism.header />
    @endif
@elseif(Route::current()->getName() == 'frontend.business_owner.index')
    <x-frontend.merchant.header />
@else
        ...
        <div class="container top-header">
            ...
            <nav class="navbar navbar-expand-lg header-m">
                ...
                <a class="navbar-brand" href="{{ route('frontend.index') }}"><img
                        src="{{ asset('frontend_assets/images/logo.svg') }}"></a>
                ...
                <div class="collapse navbar-collapse" id="navbarSupportedContent">
                    ...
                    <ul class="navbar-nav ms-auto top-navication">
                        ...
                        <li class=""><button class="join-today-button">Join Today</button></li>
                        <li class=""><a href="{{ route('frontend.business_owner.index') }}" target="_blank">For
...
<body>
        ...
        <div class="modal-dialog homemodal modal-lg">
            <div class="modal-content">
                <div class="modal-body">
                    ...
                    <div class="mt-5 mb-3 popup-logo">
                        <img src="{{ asset('frontend_assets/images/logosmart-reward.svg') }}" />
                    </div>  return response()->json(['success' => true, 'status' => 0, 'message' => 'Consumer saved successfully', 'message1' => $message1, 'message2' => $message2]);
                            ...
                            <div class="row">
                                ...
                                <table>
                                    <tr>
                                        ...
                                        <td id="latter_count_green">
                                            <span id="latter_count">0</span>/8
                                        </td>
                                    </tr>
                                    <tr>
                                        <td>
    ...
    @if (!empty(Session::get('error_code')))
        @if (Session::get('error_code') != 'open modal')
            <script>
    ...
    @if (Session::has('error'))
        ...
        @elseif(Session::has('type'))
            ...
            @elseif(Session::get('type') == 'register_password')
                ...
                @elseif(Session::get('type') == 'consumer_invitation')
                <script>
                    $(function() {
                        $('#new-registration-modal').modal('show');
                        $('.registration_input').css('display', 'none')
    ...
    @if (Session::has('token'))
        @if ((Session::get('token') != '') && (Session::get('user_type') != 'merchant'))
            <script>
                $(function() {
                    var token = '<?php echo Session::get('token'); ?>';
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('.next-btn-email-check').click(function(){
          
                var email = $("#consumer_email_address").val();
                // console.log(data);
                $.ajax({
                    url: "{{ route('frontend.consumer-email-checking') }}",
                    type: 'get',
                    data: {
                        email: email,
                    },
                    success: function(result) {
                        console.log(result);
                        if (result.status == 1) {
                            $("#consumeremailerror").html("");
                            setTimeout(() => {
                                toastr.error(
                                    'You are already registered as a Consumer. Please login to continue.');  
                                },
                             500);
                             $('#new-registration-modal').modal('hide');
                            ...
                            $('.registration_input').slideToggle(); 
                             $('.next-btn-email-check').css('display', 'none'); 
                        }else{
                            $("#consumeremailerror").html('Please enter the email').css('color',
                                'red');
                        }
        ...
        $(document).ready(function() {
            ...
            $("#new_registration_submit").validate({
                ...
                messages: {
                    ...
                    consumer_phone: {
                        ...
                        maxlength: "minimum length should be 11"
                    },
                    consumer_zipcode:{
                        required: "please enter your zipcode",
                        number: "zipcode should be a number",
            ...
            $("#new_registration_submit").submit(function(e) {
                e.preventDefault(e);
                console.log($("#consumer_zipcode").val());
                $.ajaxSetup({
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    }
                ...
                $.ajax({
                    ...
                    data: {
                        ...
                        consumer_zipcode: $("#consumer_zipcode").val(),
                        req_badge_id: $("#req_badge_id").val(),
                    },
                    success: function(result) {
                        ...
                        // }
                           if(result.status == 2) {
                            $('#new-registration-model').find('form').trigger('reset');
                            $("#new-registration-modal").modal('hide');
                        ...
                        //     $("#unitmodal").modal('show');
                         // }
                            ...
                            $("#consumeremailerror").html('Email has already been used').css('color',
                                'red');
                        } else if (result.status == 9) {
            ...
            $("#consm_create_password").keyup(function(e) {
                if (e.target.value.length <= 8) {
                    $("#latter_count").text(e.target.value.length);

                }
                if (e.target.value.length >= 8) {
                    $('#latter_count_green').css('color', 'green')
                } else {
                    $('#latter_count_green').css('color', 'black')

                }

                let upper = containsUppercase(e.target.value);

                if (upper == true) {
                    $('#upper').text(1)
                    $('#upper_green').css('color', 'green')
                } else {
                    $('#upper').text(0)
                    $('#upper_green').css('color', 'black')
                }
            });
            ...
            $("#password_submit").submit(function(e) {
                ...
                if($("#consumer_password").val() != ''){
                    ...
                    $.ajax({
                        url: "{{ route('frontend.new-password') }}",
                        type: 'POST',
                        data: {

<x-admin.table>
    ...
    <x-slot name="thead">
        <tr role="row">
            ...
            <th class="align-center" tabindex="0" aria-controls="kt_table_1" rowspan="1" colspan="1" style="width: 15%;"
                aria-sort="ascending" aria-label="Agent: activate to sort column descending">Type <i
                    class="fa fa-fw fa-sort pull-right" style="cursor: pointer;"
                    wire:click="sortBy('travel_tourism_id')"></i>
            </th>

            <th class="align-center" tabindex="0" aria-controls="kt_table_1" rowspan="1" colspan="1" style="width: 15%;"
                aria-sort="ascending" aria-label="Agent: activate to sort column descending">Travel & Tourism Name <i
                    class="fa fa-fw fa-sort pull-right" style="cursor: pointer;"
                    wire:click="sortBy('travel_tourism_id')"></i>
            </th>
        ...
        <tr class="filter">
            ...
            <th>
                <select class="form-control form-control-sm form-filter kt-input" wire:model.defer="searchType"
            ...
            <th>
                <x-admin.input type="search" wire:model.defer="searchTravelName" placeholder="" autocomplete="off"
                    class="form-control-sm form-filter" />
            </th>
            <th>
                {{--
                ...
                <select class="form-control form-control-sm form-filter kt-input" wire:model.defer="searchTitle"
                    title="Select" data-col-index="2">
                    <option value="">Select One</option>
                    <option value="Associate">Associate</option>
                    <option value="Manager">Manager</option>
                    <option value="Corporate Lead">Corporate Lead</option>
                    <option value="Corporate System Admin">Corporate System Admin</option>
                </select>
            </th>
    ...
    <x-slot name="tbody">
        ...
        <tr role="row" class="odd">
            <td class="sorting_1 align-center" tabindex="0">
                <div class="kt-user-card-v2">
                    ...
                    <div class="kt-user-card-v2__details">
                        ...
                        <span class="kt-user-card-v2__email ">Member since
            ...
            {{-- @dd( $user->travelType) --}}
            @if($user->travelType)
            <td class="align-center">{{ $user->travelType->travel_tourism_type == 'Short Rental' ? 'Short Term Rental' :
                'Hotel/Resort' }}</td>
            <td class="align-center">{{ $user->travelType->name }}</td>
            @else
            ...
            <td class="align-center"><span
                    ...
                    wire:click="changeStatusConfirm({{ $user->id }})">{{ $user->active == 1 ? 'Active' : 'Inactive'
                    }}</span>
            </td>
            <x-admin.td-action>
                <a class="dropdown-item" href="{{ route('admin.travel_tourism.useredit', ['id' => $user->id]) }}"><i
                        class="la la-edit"></i> Edit</a>
                {{-- <a class="dropdown-item"
                    href="{{ route('admin.provider.id-verification', ['id' => $user->id]) }}"><i
                        class="fas fa-user-check"></i> Id verification</a>
                <a class="dropdown-item" href="{{ route('admin.provider-user.property', ['id' => $user->id]) }}"><i
                        class="fas fa-user-check"></i> Property List</a> --}}
        ...
        {{-- <tr>
            ...
            <x-admin.td-action>
                <a class="dropdown-item" href="{{route('provider-user.edit', ['user' => $user->id])}}"><i
                        class="la la-edit"></i>
                    Edit</a>
                <button href="#" class="dropdown-item" wire:click="deleteAttempt({{ $user->id }})"><i
                        class="fa fa-trash"></i> Delete</button>

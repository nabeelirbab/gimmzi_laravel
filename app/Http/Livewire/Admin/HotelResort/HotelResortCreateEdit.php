...
class HotelResortCreateEdit extends Component
{
    use WithFileUploads;
    use AlertMessage;
    public $address, $city, $state, $zip_code, $points_to_distribute , $status,$name, $hotel_resort;
    ...
    public function mount($hotel_resort = null)
    {
        
        if ($hotel_resort) {
            $this->hotel_resort = $hotel_resort;
            $this->fill($this->hotel_resort);
            $this->isEdit = true;
        } else
      
            $this->hotel_resort = new TravelTourism();
        ...
        $this->model_image = Media::where(['model_id' => $this->hotel_resort->id, 'collection_name' => 'hotelResortPhoto'])->first();
    ...
    public function validationRuleForUpdate(): array
    {
            ...
            return
                [
                    ...
                    'status' => ['required'],
                    'name' => ['required'],
                    'phone' => ['nullable',Rule::unique('travel_tourisms')->ignore($this->hotel_resort->id),'max:12','min:10'],
    ...
    public function saveOrUpdate()
    {
        ...
        $this->hotel_resort->fill($this->validate($this->isEdit ? $this->validationRuleForUpdate() : $this->validationRuleForSave()))->save();
        $this->hotel_resort->travel_tourism_type = 'Hotel-Resort';
        $this->hotel_resort->save();
        if ($this->photo) {
            if ($this->imgId) {
                $item = Media::find($this->imgId);
                $item->delete(); // delete previous image in the database

                $this->hotel_resort->addMedia($this->photo->getRealPath())
        ...
        $msgAction = 'Hotel/Resort was ' . ($this->isEdit ? 'updated' : 'added') . ' successfully';
        $this->showToastr("success", $msgAction);

        return redirect()->route('hotel-resorts.index');
    }
    ...
    {
        return view('livewire.admin.hotel-resort.hotel-resort-create-edit');
    }
}

<div>
    @push('style')
        <style>
            ...
            .search_frm_wrap ul li {
                background: lavender;
                padding: 4px;
                margin-bottom: 1px;
            }

            .search_frm_wrap ul li:nth-child(even) {
                background: cadetblue;
                color: white;
            }

            .search_frm_wrap ul li:hover {
                cursor: pointer;
            }
        </style>
    @endpush
    <section class="search_portal">
        <div class="container">
            <div class="seacrh_head">
                <h1>Search Travel and Tourism</h1>
                {{-- <a href="{{ route('frontend.request-info')}}" style="margin-left: 83%;" class="btn btn-primary mb-4">Request Info</a> --}}
            </div>
            <div class="search_frm_wrap">
                {{-- <form > --}}
                <div class="search_field_wrap">
                    <input type="text" class="searchtype" placeholder="Search Travel and Tourisum company"
                        wire:model.defer="search_travel_tourism" wire:keyup="autocompleteTravelTourism">

                </div>
                {{-- <div class="searchBtn_wrap search_submit">
                        <input type="button">
                    </div> --}}
                @if ($showdiv)
                    <ul>
                        @if (!empty($result))
                            @foreach ($result as $result_data)
                                <li
                                    wire:click="filterTravelTourism({{ $result_data['id'] }},'{{ $result_data['type'] }}')">
                                    {{ $result_data['name'] }}</li>
                            @endforeach
                        @endif
                    </ul>
            ...
            <div class="destination_property_wrap">
                <form>
                    <div class="cmn_subHead destination_wrap">
                        <p>Destination:</p>

                        <select wire:change.prevent="searchListByLocation" wire:model.defer="searchByLocation">
                                    ...
                                    @foreach ($state_data as $state_key => $data)
                                        @if (count($data['listing']) > 0)
                                            @foreach ($data['listing'] as $key => $short_term)
                                                <option value="{{ $short_term['states']['id'] }}">
                                                    {{ $short_term['city'] }} , {{ $short_term['states']['name'] }}
                                                </option>
                                            @endforeach
                                        @endif
                                    @endforeach
                    ...
                    <div class="cmn_subHead property_wrap">
                        <p>Property types:</p>
                        <div class="property_option">
                            @if (count($types) > 0)
                                @foreach ($types as $datas)
                                    {{-- @dd($type_ids) --}}
                                    <label>

                                        <input type="checkbox" checked name="property{{ $datas->id }}"
                                            wire:click="selectType({{ $datas->id }})" id="{{ $datas->id }}">
            ...
            <div class="qs-result">
                <div class="qs-result-scroll lg">
                    <div class="row">
                        {{-- @dd($state_travels) --}}
                        <?php //echo "<pre>"; print_r($state_travels);?>
                        @if (count($state_travels) > 0)
                            @foreach ($state_travels as $travel_data)
                                @foreach ($travel_data as $travel_key => $data)
                                    @if ($data['travel_tourism_type'] == 'Short Rental')
                                        @if (count($data['listing']) > 0)
                                            @foreach ($data['listing'] as $key => $short_term)
                                                <div class="col-lg-3 col-md-4 col-sm-6">
                                                    <div class="qs-result-box">
                                                        <div class="qs-result-image">
                                                            <div class="hotel_slider_main">
                                                                <div class="hotel_slider">
                                                                    @if ($short_term['main_img'])
                                                                        <div class="hotel_item">
                                                                            <img src="{{ $short_term['main_img'] }}"
                                                                                alt="">
                                                                        </div>
                                                                    @endif
                                                                    @if (count($short_term['photo_img']) > 0)
                                                                        @foreach ($short_term['photo_img'] as $photo)
                                                                            <div class="hotel_item">
                                                                                <img src="{{ $photo }}"
                                                                                    alt="photo">
                                                                            </div>
                                                            ...
                                                            <div class="qs-result-image-overlay extra_img_wrap">
                                                                <div class="qs-wishlist">
                                                                    <label class="media-save-lbl">
                                                                        ...
                                                                        <input type="checkbox" <?php if ($short_term['favourite_travel'] == 1) {
                                                                            echo 'checked';
                                                                        } else {
                                                                            echo 'disabled';
                                                                        } ?>>
                                                                        <span
                                                                            wire:click='saveList({{ $short_term['id'] }})'>
                                                                ...
                                                                <div class="qs-share">
                                                                    <a href="Javascript:void(0);"
                                                                        wire:click="shareListing({{ $short_term['id'] }})">
                                                        ...
                                                        <div class="qs-result-info">
                                                            <a
                                                                href="{{ route('frontend.short-term-website', base64_encode($short_term['id'])) }}">
                                                                <h3>{{ $short_term['name'] }}</h3>
                                                                <span>{{ $short_term['type']['name'] }}</span>
                                                                <ul>
                                                                    <li>Bed: {{ $short_term['no_of_bedrooms'] }}</li>
                                                                    <li>Bath: {{ $short_term['no_of_baths'] }}</li>
                                                                    <li>Guests: {{ $short_term['no_of_guests'] }}</li>
                                                                </ul>
                                                            </a>
                                                        </div>
                                                    </div>
                                    ...
                                    @else
                                        <div class="col-lg-3 col-md-4 col-sm-6">
                                            <div class="qs-result-box">
                                                ...
                                                <div class="qs-result-info">
                                                    <a href="{{ route('frontend.hotel_resort.hotel-resort-website', base64_encode($data['id'])) }}">
                                                        <h3>{{ $data['name'] }}</h3>
                            ...
                            @endforeach
                        @else
                            <div>No travel & tourism found</div>
                        @endif
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-6">
                                <div class="share-property-option-box">
                                    <input type="hidden" class="page_link" id="myInput">
    ...
    <div wire:ignore.self class="modal extra-listingWebsite-modal mailBoxModal fade" id="mailBoxModal" tabindex="-1"
        aria-labelledby="shareModalLabel" aria-hidden="true">
        ...
        <script>
            ...
            document.addEventListener('livewire:load', function(event) {
                @this.on('shareOnSocialMedia', data => {
                    ...
                    $(".page_link").val(data.url);

                    $(".whatsapp_link").attr('href', 'https://wa.me/?text=' + data.url);
                    $(".facebook_link").attr('href', 'https://www.facebook.com/sharer/sharer.php?u=' + data
                        .url);
                    $(".twitter_link").attr('href', 'https://twitter.com/intent/tweet?text=' + data.url);
                    $(".linkedin_link").attr('href',
                        'https://www.linkedin.com/sharing/share-offsite?mini=true&url=' + data.url +
                        '&title=&summary=');
                });
                ...
                $(document).ready(function() {
                    $(document).on('show.bs.modal', '.modal', function() {
                        ...
                        setTimeout(() => $('.modal-backdrop').not('.modal-stack').css('z-index',
                ...
                @this.on('favoriteLogin', function() {
                    $("#loginModal").modal('show');
                    $("#favourite_modal").modal('hide');
                    sessionStorage.setItem("get_page", "travel_website_page");

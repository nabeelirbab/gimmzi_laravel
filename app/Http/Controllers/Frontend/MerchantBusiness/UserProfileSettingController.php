...
class UserProfileSettingController extends Controller
{
    ...
    {
        $user = User::with('merchantTitle')->where('id', Auth::user()->id)->first();
        return view('frontend.merchant_owner.user-profile-setting.user_profile_settings', compact('user'));
    }
            ...
            if ($request->full_name != '') {
                if ($request->email != '') {
                    ...
                    if ($validator->fails()) {
                        ...
                        if ($request->phone != '') {
                            $validator  =   Validator::make($request->all(), [
                                "phone"  =>  'numeric|digits_between:8,15|unique:users,email,' . $request->user_id
                            ]);
                            if ($validator->fails()) {
                                ...
                                if ($user) {
                                    ...
                                    if ($request->hasFile('user_photo')) {
                                        ...
                                        if ($validator->fails()) {
                                            ...
                                            $getphoto = Media::where(['model_id' => $user->id, 'collection_name' => 'merchantImages'])->first();
                                            if($getphoto){
                                                $getphoto->delete();
                                            }

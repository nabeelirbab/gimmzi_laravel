...
<x-admin.table>
    ...
    <x-slot name="thead">
        <tr role="row">
            ...
            <th tabindex="0" aria-controls="kt_table_1" rowspan="1" colspan="1" style="width: 22%;"
                aria-sort="ascending" aria-label="Agent: activate to sort column descending" class="align-center">Service Name
                <i class="fa fa-fw fa-sort pull-right" style="cursor: pointer;" wire:click="sortBy('service_name')"></i>
            </th>

           
            <th tabindex="0" aria-controls="kt_table_1" rowspan="1" colspan="1" style="width: 12%;"
                aria-label="Company Agent: activate to sort column ascending" class="align-center"> Category Name <i
                    class="fa fa-fw fa-sort pull-right" style="cursor: pointer;" wire:click="sortBy('category_name')"></i></th>
    ...
    <x-slot name="tbody">
        @forelse($types as $data)
            <tr role="row" class="odd">
                ...
                <td class="sorting_1 align-center" tabindex="0">{{ $data->service_name }}</td>               
            ...
            <x-admin.td-action>
                <a class="dropdown-item" href="{{ route('service-type.edit', ['service_type' => $data->id]) }}"><i class="la la-edit"></i> Edit</a>
            </x-admin.td-action>
            </tr>
        @empty
            <tr>
    ...
    <x-slot name="showingEntries">
        Showing {{ $types->firstitem() ?? 0 }} to {{ $types->lastitem() ?? 0 }} of {{ $types->total() }}
        entries
    </x-slot>
</x-admin.table>

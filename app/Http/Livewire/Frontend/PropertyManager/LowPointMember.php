...
class LowPointMember extends Component
{
    ...
    public function yesAddPointToMember(){
        if($this->count_member > 0){
            $provider = Provider::find($this->property_id);
            if($provider->points_to_distribute > ($this->count_member * $this->amount)){
                foreach($this->mergedData as $data){
                    $badge_guest = ApartmentGuestBadge::where('user_id',$data['consumer_id'])->get();
                    foreach($badge_guest as $guest){
                        $apartment_guest = ApartmentGuestBadge::find($guest->id);
    ...
    public function addPointToConsumer(){
        if($this->consumerid  != ''){
            $provider_setting = ProviderLimitSetting::where('property_id', $this->property_id)->first();
                if($provider_setting){
                    if($provider_setting->add_point != null){
                        ...
                        if($this->property->points_to_distribute > $setting_point ){
                            ...
                            $point->sign = '+';
                            $point->save();
                            $badge_guest = ApartmentGuestBadge::where('user_id',$this->consumerid)->get();
                            foreach($badge_guest as $guest){
                                $apartment_guest = ApartmentGuestBadge::find($guest->id);
    ...
    public function viewconsumerProfile(){
        if($this->consumerid  != ''){
            redirect()->route('frontend.property.consumer.view.profile',$this->consumerid);
        }
        else{
            $this->emit('sucesspopup',['msg' => 'Please select a consumer first']); 
        }
    ...
    public function render()
    {
        ...
        $units = BuildingUnit::where('provider_id',$this->property_id)->pluck('id');
        $today = date('Y-m-d');
        $allbadge = Apartmentbadge::with('badge_guest','badge_guest.user')->whereIn('unit_id',$units)->where('end_date','>=',$today)->get();
         //dd($allbadge);
        if(count($allbadge) > 0){
            foreach($allbadge as $badges){
                if(count($badges->badge_guest) > 0){
                    foreach($badges->badge_guest as $guests){
                        if($guests->user_id != null){
                            if($guests->user->point <= $this->provider_settings->low_point_balance){
                                //dd($guests->badges_id);
                                $get_badges = Apartmentbadge::select('id','end_date')->where('id',$guests->badges_id)->first();
                                ...
                                $this->mergedData[] = [
                                                ...
                                                'unitid'=>$guests->badge->id,
                                                'building_name'=>$guests->badge->building->building_name,
                                                'unit'=>$guests->badge->buildingunit->unit,
        ...
        // dd($data);
        return view('livewire.frontend.property-manager.low-point-member',[
            'users' => $data,
        ]);

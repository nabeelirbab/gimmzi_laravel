<x-admin.table>
    ...
    <x-slot name="thead">
        <tr role="row">
            <th  class="align-center" tabindex="0" aria-controls="kt_table_1" rowspan="1" colspan="1" style="width: 18%;"
                aria-sort="ascending" aria-label="Agent: activate to sort column descending">ProviderId <i
                    class="fa fa-fw fa-sort pull-right" style="cursor: pointer;" wire:click="sortBy('providerId')"></i>
            </th>
            <th class="align-center" tabindex="0" aria-controls="kt_table_1" rowspan="1" colspan="1" style="width: 18%;"
                aria-sort="ascending" aria-label="Agent: activate to sort column descending">Provider Type 
            </th>
        ...
        @forelse($providers as $provider)
            <tr role="row" class="odd">
                ...
                <td class="align-center"><span
                        class="kt-badge  kt-badge--{{ $provider->status == 1 ? 'success' : 'warning' }} kt-badge--inline kt-badge--pill cursor-pointer"
                        wire:click="changeStatusConfirm({{ $provider->id }})">{{ $provider->status == 1 ? 'Active' : 'Inactive' }}</span>
                </td>
                <x-admin.td-action>
                    <a class="dropdown-item" href="{{ route('providers.edit', ['provider' => $provider->id]) }}"><i
    ...
    <x-slot name="showingEntries">
        Showing {{ $providers->firstitem() ?? 0 }} to {{ $providers->lastitem() ?? 0 }} of
        {{ $providers->total() }} entries
    </x-slot>
</x-admin.table>
